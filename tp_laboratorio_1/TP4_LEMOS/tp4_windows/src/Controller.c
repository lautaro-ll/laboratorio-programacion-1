...
int controller_removeEmployee(LinkedList* pArrayListEmployee)
{
    ...
    if(pArrayListEmployee!=NULL && controller_ListEmployee(pArrayListEmployee)
    	&& utn_getNumeroInt(&id,"Ingrese el id del empleado a eliminar: ","Error, reingrese un id valido", 0, 1000000,2)==0)
    {
            empleado = employee_find(pArrayListEmployee,id);
...
int controller_setEmployee(LinkedList* pArrayListEmployee)
{
    ...
    if(pArrayListEmployee!=NULL && controller_ListEmployee(pArrayListEmployee)
    	&& utn_getNumeroInt(&id,"Ingrese el id del empleado a modificar: ","Error, reingrese un id valido\n", 0, 1000000,2)==0)
    {
            empleado = employee_find(pArrayListEmployee,id);
            ...
            if(empleado!=NULL)
            {
                		...
                		&& (confirm=='s' || confirm=='S'))
                {
            		do
            		{
						...
						if(utn_getNumeroInt(&opc,"Seleccione dato a modificar:\n"
															"1. Nombre\n"
															"2. Horas Trabajadas\n"
															"3. Sueldo\n"
															"4. Salir\n",
															"Error, reingrese opcion (1 a 4)\n", 1, 4, 1)==0)
						{
							...
							{
							case 1:
								utn_getString(nombre,"Ingrese Nombre: ","Error (3 a 128 caracteres)\n",3,128,2);
								strcpy(empleado->nombre,nombre);
								break;
							case 2:
								utn_getNumeroInt(&horasTrabajadasAux,"Ingrese Horas Trabajadas: ", "Error (Máximo 100000000hs.)\n",0,100000000,2);
								empleado->horasTrabajadas=horasTrabajadasAux;
								break;
							case 3:
								utn_getNumeroInt(&sueldoAux,"Ingrese Sueldo: ", "Error (Máximo 1000000)\n",0,1000000,2);
                    ...
                    if(utn_getCharacter(&confirm,"¿Confirma la modificacion? ('s' para confirmar)\n","Error ('s' para confirmar)\n",'A','z',1)==0
                    		&& (confirm=='s' || confirm=='S'))
                    {
						index = ll_indexOf(pArrayListEmployee,empleado);
...
/** \brief Mover empleado y reasignar id
 *
 * \param path char*
 * \param pArrayListEmployee LinkedList*
 * \return int
...
int controller_moveEmployee(LinkedList* pArrayListEmployee)
{
    ...
    {
        if(pArrayListEmployee!=NULL && controller_ListEmployee(pArrayListEmployee)
        	&& utn_getNumeroInt(&pos,"Ingrese la posicion del empleado a mover: ","Error, reingrese un id valido\n", 0, ll_len(pArrayListEmployee),2)==0)
        {

            empleado = ll_pop(pArrayListEmployee, pos);
            printf("ID             NOMBRE      HORAS         SUELDO\n");
            printf("--------------------------------------------------\n");
            mostrarEmpleado(empleado);

            if(utn_getNumeroInt(&pos,"Ingrese posicion final: ", "Error (Debe ser mayor a 0 y menor al tamaño de la lista)\n",0,ll_len(pArrayListEmployee),2)==0)
			{
				idAux = lastId(pArrayListEmployee)+1;
				empleado->id=idAux;
				retorno=0;
           	}
            ll_push(pArrayListEmployee, pos, empleado);
